...
class DAQ_1DViewer_Omnidriver(DAQ_Viewer_base):
    ...
    def ini_detector(self, controller=None):
        ...
        try:
            ...
            if self.settings.child(('controller_status')).value() == "Slave":
                ...
                omnidriver_path = self.settings.child('omnidriver_path').value()
            ...
            for ind_spectro in range(N):
                ...
                self.x_axis = [Axis(data=wavelengths, label="Wavelength", units="m")]
                self.emit_x_axis()

                data_init.append(DataFromPlugins(name=name, data=[np.zeros_like(wavelengths)], dim='Data1D',
                                                 x_axis=Axis(data=wavelengths, label='Wavelength', units='m')))
                for ind in range(
                        2):  # this is to take into account that adding it once doen't work (see pyqtgraph Parameter...)
                    try:
    ...
    def get_xaxis(self, ind_spectro):
        ...
        wavelengths = np.array([wavelengths_chelou[ind] for ind in range(len(wavelengths_chelou))]) * 1e-9
